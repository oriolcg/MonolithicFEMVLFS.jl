...
for i in 1:10
  ...
  push!(Yago_data, CSV.File(datadir("Yago_data","lambda_L_$α.csv");header=false))
end
Fu_data = []
for case in ["L_04" "L_06" "L_08"]
  push!(Fu_data, CSV.File(datadir("Fu_data","$case.csv");header=false))
end
...
savefig(plt1,plotsdir("YagoFreq","L_factor_04"))
...
savefig(plt1,plotsdir("YagoFreq","L_factor_06"))
...
savefig(plt1,plotsdir("YagoFreq","L_factor_08"))
