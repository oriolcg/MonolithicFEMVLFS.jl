module Khabakhpasheva_time_experiment
using DrWatson
...
function run_Khabakhpasheva_time_case(case::Khabakhpasheva_time_domain_params)
  case_name = savename(case)
  println("-------------")
  println("Case: ",case_name)
  t,x,ηt = run_Khabakhpasheva_time_domain(case)
  case_name_suffix = savename(case,"jld2";digits=8)
  file = datadir("KhabakhpashevaTime", case_name_suffix)
...
ny = 1
order = 2
path = datadir("KhabakhpashevaTimeDomain")
...
produce_or_load(path,case,run_Khabakhpasheva_time_case;digits=8)

# Case 1: with joint
case = Khabakhpasheva_time_domain_params(name="xi-0")
data, file = produce_or_load(path,case,run_Khabakhpasheva_time_case)
...
xlabel!("x/L")
ylabel!("|η|/η₀")
savefig(plt1, plotsdir("KhabakhpashevaTimeDomain","with_joint"))
...
xlabel!("x/L")
ylabel!("|η|/η₀")
savefig(plt2, plotsdir("KhabakhpashevaTimeDomain","without_joint"))
end
